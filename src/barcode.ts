...
/**
 * Код товара из того, что прочитал сканер.
 *
 * Сканер читает любой код в кадре, а товар — это не любой код:
 *  • QR со ссылкой на сайт производителя стоит на этикетке рядом со штрихкодом.
 *    Он попадал в карточку товара вместо кода — касса потом искала «http://…»;
 *  • DataMatrix «Честного знака» (вода, молочка, одежда) несёт в себе штрихкод
 *    товара: «01» + GTIN из 14 цифр, дальше серийный номер экземпляра. Сам код
 *    уникален для каждой бутылки, и искать по нему товар бессмысленно — нужен GTIN;
 *  • ручные сканеры добавляют в начало символы вида «]d2» — признак формата.
 */
export const extractProductCode = (raw: unknown): { code: string } | { error: string } => {
  let code = normalizeBarcode(raw).replace(/^\][A-Za-z]\d/, '');
  if (!code) return { error: 'Пустой код' };

  if (/^[a-z][a-z0-9+.-]*:\/\//i.test(code) || /^www\./i.test(code)) {
    return { error: 'Это ссылка из QR-кода, а не штрихкод товара. Наведите камеру на полоски штрихкода.' };
  }

  const gs1 = code.match(/^01(\d{14})/);
  if (gs1 && isValidGtin(gs1[1])) {
    // GTIN-14 с ведущим нулём — это обычный EAN-13 на упаковке.
    code = gs1[1].startsWith('0') ? gs1[1].slice(1) : gs1[1];
  }
  return { code };
};
