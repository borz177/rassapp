...
/**
 * Порядковые номера договоров — по дате оформления, от первого к последнему.
 *
 * Своего номера у договора не хранится: он всегда был «пятидесятым по счёту» и
 * ничем иным. Поэтому номер считается из списка, а не выдаётся при создании, —
 * и одинаково выходит везде, где его показывают: в списке договоров, в журнале
 * склада, в печатной форме. Считать его на месте в каждом экране значило бы
 * получить три разных номера у одной бумаги.
 *
 * Дата, а не порядок в массиве: договор, заведённый задним числом, встаёт на
 * своё место в истории, а не в конец списка.
 */
export const contractNumbers = (
  sales: { id: string; startDate: string }[]
): Record<string, string> => {
  const map: Record<string, string> = {};
  [...sales]
    .sort((a, b) => {
      const diff = new Date(a.startDate).getTime() - new Date(b.startDate).getTime();
      // При одинаковой дате порядок задаёт id: он начинается с метки времени
      // создания, так что договоры одного дня идут как их и оформляли.
