...
const SaleDetailsModal = ({ sale, customerName, onClose, appSettings }: { sale: Sale, customerName: string, onClose: () => void, appSettings: AppSettings }) => {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-gradient-to-br from-slate-900/80 to-indigo-900/60 backdrop-blur-md animate-in fade-in zoom-in duration-300" onClick={onClose}>
            <div className="bg-white/95 backdrop-blur-sm w-full max-w-md rounded-3xl shadow-2xl overflow-hidden border border-white/20" onClick={e => e.stopPropagation()}>
        ...
        {activeTab === 'overview' && (
            <div className="space-y-6 animate-in fade-in duration-500">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                  <div className="group bg-white p-6 rounded-2xl shadow-sm hover:shadow-xl transition-all duration-300 border border-slate-100 hover:border-emerald-200 hover:-translate-y-1">
                    <p className="text-sm font-medium text-slate-500 flex items-center gap-2">
                      <span className="w-2 h-2 bg-emerald-500 rounded-full"></span>
                      Собрано средств
                    </p>
                    <p className="text-2xl font-bold text-emerald-600 mt-2">{formatCurrency(calculatedStats.totalRevenue, appSettings.showCents)} ₽</p>
                  ...
                  <div className="group bg-white p-6 rounded-2xl shadow-sm hover:shadow-xl transition-all duration-300 border border-slate-100 hover:border-amber-200 hover:-translate-y-1">
                    <p className="text-sm font-medium text-slate-500 flex items-center gap-2">
                      <span className="w-2 h-2 bg-amber-500 rounded-full"></span>
                      Долг клиентов
                    </p>
                    <p className="text-2xl font-bold text-amber-600 mt-2">{formatCurrency(calculatedStats.totalOutstanding, appSettings.showCents)} ₽</p>
                    <div className="mt-3 h-1 w-full bg-amber-100 rounded-full overflow-hidden">
                      <div className="h-full w-2/3 bg-amber-500 rounded-full group-hover:w-full transition-all duration-500"></div>
                    </div>
                  </div>
                  <div className="group bg-white p-6 rounded-2xl shadow-sm hover:shadow-xl transition-all duration-300 border border-slate-100 hover:border-blue-200 hover:-translate-y-1">
                    <p className="text-sm font-medium text-slate-500 flex items-center gap-2">
                      <span className="w-2 h-2 bg-blue-500 rounded-full"></span>
                      Оборотные средства
                    </p>
                    <p className="text-2xl font-bold text-blue-600 mt-2">{formatCurrency(currentWorkingCapital, appSettings.showCents)} ₽</p>
                    <div className="mt-3 h-1 w-full bg-blue-100 rounded-full overflow-hidden">
                      <div className="h-full w-5/6 bg-blue-500 rounded-full group-hover:w-full transition-all duration-500"></div>
                    </div>
                  </div>
                  <div className="group bg-white p-6 rounded-2xl shadow-sm hover:shadow-xl transition-all duration-300 border border-slate-100 hover:border-indigo-200 hover:-translate-y-1">
                    <p className="text-sm font-medium text-slate-500 flex items-center gap-2">
                      <span className="w-2 h-2 bg-indigo-500 rounded-full"></span>
                      Продажи в рассрочку
                    </p>
                    <p className="text-2xl font-bold text-indigo-600 mt-2">{formatCurrency(calculatedStats.installmentSalesTotal, appSettings.showCents)} ₽</p>
                    <div className="mt-3 h-1 w-full bg-indigo-100 rounded-full overflow-hidden">
                      <div className="h-full w-4/5 bg-indigo-500 rounded-full group-hover:w-full transition-all duration-500"></div>
                    </div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  {/* Recent Contracts */}
                  <div className="bg-white/80 backdrop-blur-sm p-6 rounded-2xl shadow-sm border border-slate-100 hover:shadow-md transition-all">
                            ...
                            <div className="mt-3 h-1.5 w-full bg-slate-100 rounded-full overflow-hidden">
                              <div 
                                className={`h-full rounded-full transition-all duration-500 ${
                                  p.isTomorrow ? 'bg-amber-500' : p.isToday ? 'bg-emerald-500' : 'bg-rose-500'
                                }`}
                                style={{ width: '60%' }}
                              ></div>
                            </div>
