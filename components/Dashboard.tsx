...
const SaleDetailsModal = ({ sale, customerName, onClose, appSettings }: { sale: Sale, customerName: string, onClose: () => void, appSettings: AppSettings }) => {
    ...
    const status = statusMap[sale.status] || { label: sale.status, color: 'bg-slate-100 text-slate-700 border-slate-200 dark:border-slate-700' };
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-gradient-to-br from-slate-900/80 to-indigo-900/60 backdrop-blur-md animate-in fade-in zoom-in duration-300" onClick={onClose}>
            <div className="bg-white dark:bg-slate-900/95 backdrop-blur-sm w-full max-w-md rounded-3xl shadow-2xl overflow-hidden border border-white/20" onClick={e => e.stopPropagation()}>
                {/* Header with gradient */}
                <div className="relative p-6 bg-gradient-to-r from-indigo-600 to-indigo-500 text-white">
                    <div className="absolute top-0 right-0 w-32 h-32 bg-white dark:bg-slate-900/10 rounded-full -mr-8 -mt-8 blur-2xl"></div>
                    ...
                    <button onClick={onClose} className="absolute top-4 right-4 w-8 h-8 bg-white dark:bg-slate-900/20 hover:bg-white dark:bg-slate-900/30 rounded-full flex items-center justify-center backdrop-blur-sm transition-all">
                ...
                <div className="p-6 space-y-4 bg-slate-50">
                    ...
                    {/* Financial details */}
                    <div className="bg-white dark:bg-slate-900 rounded-xl p-4 border border-slate-100 shadow-sm space-y-3">
                        <div className="flex justify-between items-center">
                    ...
                    {sale.paymentPlan && sale.paymentPlan.length > 0 && (
                        <div className="bg-white dark:bg-slate-900 rounded-xl p-4 border border-slate-100 shadow-sm">
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-indigo-50/30 pb-24 w-full">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 space-y-6 animate-in fade-in slide-in-from-top-4 duration-500">
        ...
        {/* Tabs */}
        <div className="flex bg-white dark:bg-slate-900/70 backdrop-blur-sm p-1.5 rounded-2xl shadow-sm border border-white">
          <button
                            ...
                            <button
                              ...
                              className={`px-5 py-2.5 rounded-full text-xs font-bold whitespace-nowrap transition-all duration-300 border ${
                                !selectedAccountId 
                                  ...
                                  : 'bg-white dark:bg-slate-900 text-slate-600 border-slate-200 dark:border-slate-700 hover:border-indigo-300 hover:text-indigo-600'
                              }`}
                            >
                                Все счета
                            </button>
...
<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
  <div className="group bg-white dark:bg-slate-900 p-5 rounded-3xl shadow-sm hover:shadow-xl transition-all duration-300 border border-slate-100 hover:border-emerald-200 hover:-translate-y-1">
  ...
  <div className="group bg-white dark:bg-slate-900 p-5 rounded-3xl shadow-sm hover:shadow-xl transition-all duration-300 border border-slate-100 hover:border-amber-200 hover:-translate-y-1">
    <div className="flex items-start gap-4">
  ...
  <div className="group bg-white dark:bg-slate-900 p-5 rounded-3xl shadow-sm hover:shadow-xl transition-all duration-300 border border-slate-100 hover:border-blue-200 hover:-translate-y-1">
    <div className="flex items-start gap-4">
  ...
  <div className="group bg-white dark:bg-slate-900 p-5 rounded-3xl shadow-sm hover:shadow-xl transition-all duration-300 border border-slate-100 hover:border-indigo-200 hover:-translate-y-1">
    <div className="flex items-start gap-4">
      ...
      <div className="flex-1">
        ...
        <p className="text-2xl font-bold text-slate-800">
          {formatCurrency(calculatedStats.installmentSalesTotal, appSettings.showCents)} ₽
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  {/* Recent Contracts */}
                  <div className="bg-white dark:bg-slate-900/80 backdrop-blur-sm p-6 rounded-3xl shadow-sm border border-slate-100 hover:shadow-md transition-all">
                      ...
                      <div className="space-y-3">
                          {lastFiveSales.length === 0 ? (
                            <div className="text-center py-8 text-slate-400 bg-slate-50 rounded-xl border border-dashed border-slate-200 dark:border-slate-700">
                              Нет договоров
                            </div>
                          ) : lastFiveSales.map((sale, idx) => (
                              <div key={sale.id} className="group flex items-center justify-between p-3 bg-slate-50 hover:bg-white dark:bg-slate-900 rounded-xl transition-all hover:shadow-md border border-transparent hover:border-indigo-100 animate-in fade-in slide-in-from-bottom-2" style={{ animationDelay: `${idx * 50}ms` }}>
                                  <div>
        ...
        {activeTab === 'upcoming' && (
          <div className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-500" onClick={() => setActiveActionMenu(null)}>
              ...
              {/* Date Filters */}
              <div className="flex gap-2 p-1 bg-white dark:bg-slate-900/70 backdrop-blur-sm rounded-xl w-fit shadow-sm">
                  <button
              ...
              {/* Payments List */}
              {upcomingAndOverduePayments.length === 0 ? (
                  <div className="text-center py-16 bg-white dark:bg-slate-900/80 backdrop-blur-sm rounded-2xl border border-dashed border-slate-200 dark:border-slate-700">
                  ...
                  <div className="space-y-3">
                    {upcomingAndOverduePayments.map((p, idx) => (
                        <div
                          key={p.sale.id}
                          className="group bg-white dark:bg-slate-900/90 backdrop-blur-sm p-5 rounded-2xl shadow-sm hover:shadow-xl transition-all duration-300 border border-slate-100 hover:border-indigo-200 relative animate-in fade-in slide-in-from-bottom-2"
                                ...
                                <div className="flex items-center gap-3">
                                    ...
                                    <div className="relative">
                                      ...
                                      {activeActionMenu === p.sale.id && (
                                          <div className="absolute right-0 top-8 bg-white dark:bg-slate-900 shadow-xl rounded-xl z-20 w-48 overflow-hidden animate-in fade-in zoom-in border border-slate-100">
                                              <button
