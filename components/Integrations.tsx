...
>
  {/* 🔹 Заголовок карточки — БЕЗ клика, БЕЗ курсора, БЕЗ ховера */}
  <div
    className={`p-5 flex justify-between items-center ${
      waEnabled ? 'bg-emerald-50/50' : 'bg-white'
    }`}
    // ← УБРАЛИ: onClick={handleCardClick}, cursor-pointer, hover:bg-*
  >
    ...
    <div className="flex items-center gap-4">
      {/* 🔹 УБРАЛИ блок с шевроном полностью */}

      {/* Переключатель — работает независимо, без setIsExpanded */}
      <div
        onClick={(e) => {
          e.stopPropagation();
          const newState = !waEnabled;
          setWaEnabled(newState);
          // ← УБРАЛИ: setIsExpanded(newState);
        }}
  ...
  {/* 🔹 Контент показывается сразу при waEnabled, без isExpanded */}
  {waEnabled && (
