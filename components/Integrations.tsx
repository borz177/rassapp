...
const Integrations: React.FC<IntegrationsProps> = ({
  ...
  return (
    <div className="space-y-6 animate-fade-in pb-20">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 transition-all duration-300">
        {/* Заголовок карточки */}
        <div className="p-5 flex justify-between items-center bg-white">
          <div className="flex items-center gap-3">
            <div className={`w-10 h-10 rounded-full flex items-center justify-center text-white ${isConnected ? 'bg-emerald-500' : 'bg-slate-300'}`}>
        ...
        {/* 🔹 Поля ввода токенов */}
        {showCredentials && (
          <div className="px-5 pb-5 space-y-4 animate-fade-in border-t border-slate-100 pt-5">
            ...
            {connectionStatus !== 'IDLE' && (
              <div className={`flex items-center gap-2 p-3 rounded-xl text-sm ${connectionStatus === 'AUTHORIZED' ? 'bg-emerald-50 text-emerald-700' : connectionStatus === 'ERROR' ? 'bg-red-50 text-red-700' : 'bg-amber-50 text-amber-700'}`}>
            ...
            <div className="flex gap-3">
              <button onClick={() => setShowCredentials(false)} className="flex-1 py-3 bg-slate-100 text-slate-600 font-bold rounded-xl hover:bg-slate-200 transition-all">
                Отмена
              </button>
              <button
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/70 backdrop-blur-sm animate-fade-in" onClick={() => setShowPreview(false)}>
          <div className="bg-white w-full max-w-md rounded-2xl shadow-2xl overflow-hidden animate-scale-in" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between p-4 border-b border-slate-100 bg-gradient-to-r from-indigo-50 to-emerald-50">
              <h3 className="font-bold text-slate-800 flex items-center gap-2"><span className="text-lg">👁️</span> Предпросмотр сообщения</h3>
            ...
            <div className="p-4 max-h-[60vh] overflow-y-auto">
              ...
              <div className="mt-4 p-3 bg-slate-50 rounded-xl border border-slate-200">
                <p className="text-[10px] text-slate-500">💡 <b>Примечание:</b> Это пример. В реальном сообщении переменные заменятся на данные клиента.</p>
              </div>
            </div>
            <div className="flex gap-3 p-4 border-t border-slate-100 bg-slate-50">
              <button onClick={() => setShowPreview(false)} className="flex-1 py-2.5 bg-slate-200 text-slate-700 rounded-xl font-medium text-sm hover:bg-slate-300 transition-colors">Закрыть</button>
