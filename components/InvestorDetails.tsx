    ...
    <div className="space-y-4 animate-fade-in pb-20">
      ...
      {activeTab === 'INFO' && (
          <div className="space-y-4 pt-2">
               <div className="bg-white dark:bg-slate-900 rounded-xl p-5 shadow-sm border border-slate-100 space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                    <div className="bg-white dark:bg-slate-900 rounded-xl p-4 shadow-sm border border-slate-100"><h3 className="font-bold text-sm text-slate-800 mb-1">Ожидаемая прибыль</h3><p className="text-xs text-slate-500 mb-2">С активных договоров</p><p className="text-2xl font-bold text-indigo-800">{formatCurrency(expectedTotalProfit, appSettings.showCents)} ₽</p></div>
                    <div onClick={() => setShowProfitDetails(true)} className="bg-white dark:bg-slate-900 rounded-xl p-4 shadow-sm border border-slate-100 cursor-pointer hover:bg-slate-50"><h3 className="font-bold text-sm text-slate-800 mb-1">Полученная прибыль</h3><p className="text-xs text-slate-500 mb-2">Общий баланс</p><p className="text-2xl font-bold text-emerald-800">{formatCurrency(totalProfitEarned * (investor.profitPercentage/100) - totalProfitWithdrawn, appSettings.showCents)} ₽</p></div>
                </div>

               <div className="bg-white dark:bg-slate-900 rounded-xl p-5 shadow-sm border border-slate-100 space-y-4">
                  <h3 className="font-bold text-slate-800">Полученная прибыль за период</h3>
                  <div className="grid grid-cols-2 gap-3">
                      <div><label className="text-xs text-slate-500 mb-1 block">Начало</label><input type="date" className="w-full p-2 border border-slate-200 dark:border-slate-700 rounded-lg" value={period.start} onChange={e => setPeriod(p => ({...p, start: e.target.value}))} /></div>
                      <div><label className="text-xs text-slate-500 mb-1 block">Конец</label><input type="date" className="w-full p-2 border border-slate-200 dark:border-slate-700 rounded-lg" value={period.end} onChange={e => setPeriod(p => ({...p, end: e.target.value}))} /></div>
                  </div>
      ...
      {showProfitDetails && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-fade-in" onClick={() => setShowProfitDetails(false)}>
              <div className="bg-white dark:bg-slate-900 w-full max-w-md rounded-2xl shadow-2xl flex flex-col max-h-[80vh]" onClick={e => e.stopPropagation()}>
                  <div className="p-5 border-b border-slate-200 dark:border-slate-700"><h3 className="text-lg font-bold text-slate-800">Детализация прибыли</h3><p className="text-sm text-slate-500">Общий баланс: <span className="font-bold text-emerald-600">{formatCurrency(totalProfitEarned * (investor.profitPercentage/100) - totalProfitWithdrawn, appSettings.showCents)} ₽</span></p></div>
