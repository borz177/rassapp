...
const App: React.FC = () => {
  ...
  const checkAccess = (feature: 'WRITE' | 'INVESTORS' | 'AI' | 'WHATSAPP' | 'EMPLOYEES'): boolean => {
      ...
      if (isEmployee || isInvestor || user.role === 'admin') return true; // Manager handles sub-user permissions separately, assuming subscription check on manager applies or is skipped for view-only
  ...
  const handleUpdateSettings = async (newSettings: AppSettings) => {
      ...
      if (user) {
          // Save with a predictable ID so it overwrites
