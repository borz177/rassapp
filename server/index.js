...
const VALID_DATA_TYPES = ['customers', 'products', 'sales', 'expenses', 'accounts', 'investors', 'partnerships', 'suppliers', 'settings', 'tasks', 'stockMovements', 'retailSales', 'warehouses'];
...
app.get('/api/data', auth, async (req, res) => {
  try {
    ...
    const result = {
      ...
      stockMovements: [], retailSales: [], warehouses: [], settings: null
    };
...
// ✅ Версия Android-приложения (APK), КОТОРАЯ ЛЕЖИТ НА РАЗДАЧЕ в /downloads/finuchet.apk.
//
// Увеличивать не тогда, когда подняли versionCode в android/app/build.gradle, а тогда, когда
// новый APK реально выложен. Иначе получается петля, в которую приложение и попало: здесь
// стояло 3, а на раздаче лежал APK от 29 июля с кодом 2. Каждому пользователю показывалось
// «доступно обновление», он скачивал тот же самый файл, ничего не менялось — и при следующем
// запуске окно появлялось снова.
//
