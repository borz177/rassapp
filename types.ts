...
export interface UserPermissions {
  ...
  canDelete: boolean;
  /**
   * Доступ к магазину и складу. Отдельно от CRUD: те права отвечают на вопрос
   * «что сотруднику можно делать с записью», а этот — «пускать ли его в этот
   * раздел вообще». Продавцу за прилавком незачем видеть договоры рассрочки,
   * а менеджеру по рассрочке — остатки и закупочные цены.
   *
   * Пусто у сотрудников, заведённых до появления магазина: доступа у них не
   * было, и включать его молча нельзя.
   */
  canUseShop?: boolean;
  /**
   * Склады магазина, с которыми работает сотрудник. Пусто — все склады, в том
   * числе заведённые позже: так новый склад не остаётся без продавцов, а у всех,
   * кто заведён до появления выбора, доступ не сужается молча. Действует, только
   * когда складов больше одного.
   */
...
export interface Product {
  ...
  /**
   * Артикул — внутреннее обозначение товара. До появления штрихкодов сюда же
   * вписывали штрихкод, поэтому поиск по отсканированному коду смотрит и сюда.
   */
  sku?: string;
  /**
   * Штрихкоды. Списком: у одного товара бывает несколько кодов — от разных
   * поставщиков или на штучной и групповой упаковке. Хранятся без пробелов.
   */
  barcodes?: string[];
  ...
  isArchived?: boolean;
  /**
   * Ручной порядок в каталоге. Пусто у товаров, которых ни разу не двигали, —
   * такие идут по алфавиту после расставленных. Шаг между значениями крупный
   * (1000), чтобы вставка между соседями не требовала переписывать весь список.
   */
  sortOrder?: number;
  updatedAt?: string;
  /**
...
/**
 * Деньги, полученные по розничной продаже в долг.
 *
 * Живут внутри самого чека, а не отдельной коллекцией: так платёж невозможно
 * потерять отдельно от продажи, к которой он относится, и не нужно заводить на
 * сервере новый тип данных ради двух полей. Тем же способом устроен график
 * платежей у рассрочки.
 */
export interface RetailPayment {
  id: string;
  amount: number;
  date: string;
  /** Счёт, на который фактически пришли деньги — он может отличаться от счёта чека */
  accountId: string;
  note?: string;
...
export interface RetailSale {
  ...
  isCancelled?: boolean;
  /**
   * Продажа в долг: товар отдан, деньги в кассу не поступили. Такой чек не
   * увеличивает баланс счёта — его увеличивают полученные позже платежи.
   */
  isCredit?: boolean;
  /** Полученные по долгу деньги. У обычного чека пусто: там всё получено сразу. */
...
export interface StockMovement {
  ...
  batchId?: string;
  /**
   * Номер документа — свой у каждого вида: приход, списание, перемещение и
   * инвентаризация нумеруются с единицы, как и продажи. Хранится, а не
   * считается на лету: напечатанная накладная №5 не должна стать №6 оттого,
   * что кто-то провёл документ задним числом.
   */
  docNumber?: string;
  supplierId?: string;
  /**
...
export interface Expense {
  ...
  /**
   * Приход на склад, долг за который гасит оплата партнёру: ключ документа
   * (batchId прихода). Необязателен — без него оплата гасит поставки по очереди,
   * от старых к новым (см. supplierSupplyBalances).
   */
  supplyDocId?: string;
...
export interface AppSettings {
   ...
   */
  /**
   * Печатная форма договора. Пусто — «Современный»: он был единственным, и
   ...
   * может быть подсобной частью дела, и тогда занимать им половину главного
   * экрана незачем — а может быть основным занятием, и тогда касса нужна
   * первой же кнопкой.
   */
  shopDashboardTab?: boolean;
  /**
   * Разрешить продавать в минус — брать товара больше, чем числится на складе.
   *
   * По умолчанию выключено: остаток на то и остаток, чтобы ему верили, а тихо
   * ушедший в минус товар потом не сходится ни с инвентаризацией, ни с
   * себестоимостью. Кому пересчёт делать некогда — включает и продаёт как есть,
   * система только предупреждает.
   */
  shopAllowNegativeStock?: boolean;
}
